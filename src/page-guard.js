...
export function assessPage() {
  ...
  if (WALLET.test(text)) return { stopAll: false, stopAccept: true, reason: 'Wallet-related page. Automatic acceptance is paused.' };
  for (const root of searchRoots) {
    for (const field of root.querySelectorAll('input')) {
      if (!visible(field)) continue;
      const identity = `${field.name} ${field.id} ${field.autocomplete}`;
      if (field.type === 'password' || /password|passcode|cc-number|cc-csc|card.?number|cvv|cvc|credit.?card/i.test(identity)) {
        return { stopAll: false, stopAccept: true, reason: 'Password or payment form. Automatic acceptance is paused.' };
      }
    }
    for (const frame of root.querySelectorAll('iframe')) {
      if (visible(frame) && /payment|credit.?card|checkout|wallet|stripe\.com|paypal\.com/i.test(`${frame.title} ${frame.src}`)) {
        return { stopAll: false, stopAccept: true, reason: 'Embedded payment form. Automatic acceptance is paused.' };
      }
    }
    for (const link of root.querySelectorAll('a[href], [role="button"], button')) {
      ...
      if (link.hasAttribute('download') || /\.(exe|msi|dmg|pkg|apk|crx)([?#]|$)/i.test(link.getAttribute('href') || '') || /^download\b|^install\b/i.test((link.innerText || link.textContent || '').trim())) {
        return { stopAll: false, stopAccept: true, reason: 'Software download prompt. Automatic acceptance is paused.' };
      }
    }
  }
  return { stopAll: false, stopAccept: false, reason: '' };
}
