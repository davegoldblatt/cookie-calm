...
export function shape(snapshot) {
  return JSON.stringify((snapshot.preferences || []).map(p => [p.id,p.purpose,p.grantsWhen]).sort((a,b)=>a[0].localeCompare(b[0])));
}
...
export function plan(snapshot) {
  ...
  if (snapshot.stage === 'blocked') return {type:'stop', reason:'protected'};
  ...
  const preference = preferences.find(p=>p.value !== preferenceGoal(p));
