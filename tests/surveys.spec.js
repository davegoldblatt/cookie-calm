...
async function visit(page,body,{host='survey-a.example',script=''}={}){
  await page.route(`https://${host}/**`,r=>r.fulfill({contentType:'text/html',body:`<!doctype html><main><h1>Article</h1><p id="article">Keep this article</p></main>${witness}${body}<script>window.actions=[];document.addEventListener('click',e=>{const close=e.target.closest('[data-close]');if(close){e.preventDefault();actions.push('close');close.closest('[data-survey]').remove()}else if(e.target.closest('.survey-rating-btn'))actions.push('answer')});${script}</script>`}));
  await page.goto(`https://${host}/read`);await expect.poll(()=>page.evaluate(()=>window.rejected)).toBe(true);
}
const count=worker=>worker.evaluate(async()=>Object.values(await chrome.storage.session.get(null)).reduce((n,s)=>n+(s.promotionsDismissed||0),0));
...
for(const [host,named,copy] of [['survey-a.example',true,'original'],['survey-b.example',false,'Help us improve our site']])test(`recognizes rating surveys on ${host}`,async({extension:{page,worker}})=>{
  const html=copy==='original'?survey():survey('Close',named).replace('Help us make our website better!',copy).replace("Overall, how well did Epoch's website meet your needs today?",'How satisfied are you with our website?');
...
test('independent invitation and question and rating controls are required',async({extension:{page}})=>{
  ...
  ]) {await visit(page,html);await page.waitForTimeout(900);expect(await page.evaluate(()=>actions)).toEqual([]);}
});
...
test('survey opened indirectly stays open after delayed mount and replacement',async({extension:{page}})=>{
  ...
  await page.locator('#open').click();await page.waitForTimeout(4000);await expect(page.locator('[data-survey]')).toBeVisible();await page.evaluate(()=>{const s=document.querySelector('[data-survey]');s.replaceWith(s.cloneNode(true))});await page.waitForTimeout(1700);expect(await page.evaluate(()=>actions)).toEqual([]);
});
test('a user rating preserves the survey even after the first screen is replaced',async({extension:{page,settings}})=>{
  ...
  await page.route('https://started.example/**',r=>r.fulfill({contentType:'text/html',body:`<!doctype html>${survey()}<script>window.actions=[];document.querySelector('[data-close]').onclick=()=>{actions.push('close')};document.querySelector('.survey-rating-btn').onclick=()=>{actions.push('answer');const s=document.querySelector('[data-survey]');s.replaceWith(s.cloneNode(true));document.querySelector('[data-close]').onclick=()=>actions.push('close')}</script>`}));
  await page.goto('https://started.example/');await page.locator('.survey-rating-btn').first().click();await settings({enabled:true});await page.waitForTimeout(2500);expect(await page.evaluate(()=>actions)).toEqual(['answer']);
});
...
test('reviewed Epoch survey opened by a user stays open',async({extension:{page}})=>{
  ...
  await page.locator('#open').click();await page.waitForTimeout(4000);await expect(page.locator('[data-survey]')).toBeVisible();expect(await page.evaluate(()=>actions)).toEqual([]);
});
