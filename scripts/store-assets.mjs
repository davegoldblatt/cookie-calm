...
try {
  ...
  const demos = [
    { host: 'weekend-notes.example', title: 'Less clicking.<br>More browsing.', subtitle: 'Cookies and popups, handled.', detail: 'Reject optional cookies and dismiss supported promotional prompts automatically.', text: 'A quiet corner of the internet.', result: 'reject', label: '01 / REJECT OPTIONAL COOKIES' },
    ...
    { host: 'reading-demo.example', title: 'Keep reading.<br>Skip the nags.', subtitle: 'Automatic in both cookie modes.', detail: 'Supported newsletter, donation, subscription, and app prompts close or minimize automatically.', text: 'An article to enjoy.', result: 'promotion', label: '03 / AUTOMATIC PROMPT DISMISSAL' }
  ];
  for (const [index, demo] of demos.entries()) {
    const banner = demo.result === 'promotion' ? `<section role="dialog"><h2>Join our newsletter</h2><button type="button" onclick="this.parentElement.remove()">No thanks</button></section>` : `<section id="cookie-banner" role="dialog"><p>We use cookies to improve this website.</p><button onclick="document.documentElement.dataset.result='reject';this.parentElement.remove()">Reject all</button><button onclick="document.documentElement.dataset.result='accept';this.parentElement.remove()">Accept all</button></section>`;
    await page.route(`https://${demo.host}/`, route => route.fulfill({ contentType: 'text/html', body: `<!doctype html><html lang="en"><title>Cookie Calm demonstration</title><h1>${demo.text}</h1>${banner}</html>` }));
    ...
    await worker.evaluate(async ({host, status}) => {
      for (let attempt = 0; attempt < 100; attempt++) {
        if (Object.values(await chrome.storage.session.get(null)).some(s => s.host === host && (status === 'promotion' ? s.promotionsDismissed === 1 : s.status === status))) return;
        await new Promise(resolve => setTimeout(resolve, 100));
      }
      throw new Error('The demonstration did not reach its expected status');
    }, {host: demo.host, status: demo.result === 'reject' ? 'dismissed' : demo.result});
  ...
  console.log('Created three 1280 × 800 screenshots and one 440 × 280 promotional tile.');
} finally {
  await context.close();
